...
//!
//! Note: the following test sets are not tested because this low memory implementation requires
//! seed-based private keys and has no functionality for operating on non-seed private keys.
//!
...
#[cfg(test)]
mod wycheproof {
    use crate::{MLKEMEncapsTestCase, MLKEMKeygenSeedTestCase, MLKEMTestCase, ParameterSet};
    use std::fs;
    use std::path::Path;
    use std::sync::Once;
